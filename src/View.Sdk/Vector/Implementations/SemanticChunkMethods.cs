namespace View.Sdk.Vector.Implementations
{
    ...
    /// </summary>
    public class SemanticChunkMethods : ISemanticChunkMethods
    {
        ...
        public SemanticChunkMethods(ViewSdkBase sdk)
        {
        ...
        /// <inheritdoc />
        public async Task<List<SemanticChunk>> ReadMany(
            Guid repoGuid,
            Guid docGuid,
            Guid cellGuid,
            CancellationToken token = default)
        {
            string url = _Sdk.Endpoint + "v1.0/tenants/" + _Sdk.TenantGUID + "/vectorrepositories/" + repoGuid + "/documents/" + docGuid + "/cells/" + cellGuid + "/chunks";
            return await _Sdk.RetrieveMany<SemanticChunk>(url, token).ConfigureAwait(false);
        }

        /// <inheritdoc />
        public async Task<SemanticChunk> Read(
            Guid repoGuid,
            Guid docGuid,
            Guid cellGuid,
            Guid chunkGuid,
